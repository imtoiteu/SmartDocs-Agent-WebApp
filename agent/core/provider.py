...
def get_default_provider() -> LLMProvider:
    ...
    Priority (``AGENT_LLM_PROVIDER=auto``, the default): **Groq → Gemini → local
    Qwen** — each cloud provider is included only if its key is set, and the local
    model is always the final fallback. ``AGENT_LLM_PROVIDER=local`` forces local;
    ``groq`` / ``gemini`` pin that one cloud provider (still local-backed).
    """
